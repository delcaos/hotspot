...
under `hotspot-archery-state-v5`. There are no accounts, deposits, withdrawals,
...
Every round randomizes an extreme cold/cool/warm/near miss curve from roughly
0.03× to 0.94×. Keeping every miss strictly below 1× makes the opening hotspot
worth roughly 82×–113×, depending on its location and the round profile. A shot
at a point already ruled out pays a gentler 0.96× and provides no new evidence.
Good play therefore always has 99% theoretical RTP; skill reduces the number of
wagers needed to find the point and therefore the expected absolute loss.
